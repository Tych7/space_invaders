...
def restart():
	...
	shipupgrade = pygame.image.load('shipupgrade.png').convert_alpha()
	shipupgradeleft = pygame.image.load('shipupgradeleft.png').convert_alpha()
	shipupgraderight = pygame.image.load('shipupgraderight.png').convert_alpha()
	...
	walkB = pygame.image.load('shooterA.png').convert_alpha()
	walkC = pygame.image.load('alienB.png').convert_alpha()
	...
	upgrade = pygame.image.load('upgrade.png').convert_alpha()
	lvl =  pygame.image.load('lvl7.png').convert_alpha()
	...
	while True:
		...
		if not alienP.visible:
			if not loopbreak:
				if not pause:
					for bulletH in bulletsH:
						if bulletH.y < 850 and bulletH.y > 50:
					...
					if alienP.visible:
						if len(bullets) < 1:
							...
							if not mute:
								lazer.play()
					if not alienP.visible:
						if shootloopH == 0 and shootloop == 0:
							if len(bulletsH) < 1:
								if len(bullets) < 1:
									bulletsH.append(projectileI(round(man.x + man.width //2 + 20), round(man.y + man.height//2), 6, (0,255,255)))
